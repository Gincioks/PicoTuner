...
class Attention(nn.Module):
    def __init__(self, args: MistralModelArgs):
        super().__init__()
    ...
    def forward(
        ...
        if self.flash:
            ...
            # (bs, n_heads, seqlen, cache_len + seqlen)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        x_base = x
        x = self.ffn_norm(x)
        return x_base + self.dropout(self.w2(nn.functional.silu(self.w1(x)) * self.w3(x)))


class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: MistralModelArgs):
        super().__init__()
        self.n_heads = args.n_heads
        self.dim = args.dim
        self.head_dim = args.dim // args.n_heads
...
class Transformer(nn.Module):
    def __init__(self, params: MistralModelArgs):
        super().__init__()
        self.params = params
        self.vocab_size = params.vocab_size
        self.n_layers = params.n_layers

        self.tok_embeddings = BlackboxDisk(
            nn.Embedding(params.vocab_size, params.dim), params)
        ...
        for layer_id in range(params.n_layers):
            ...
            q_lora = LoRA(attn.wq, rank=params.lora_rank, alpha=params.lora_alpha,
                          dropout=params.lora_dropout).to(params.compute_dtype)
            v_lora = LoRA(attn.wv, rank=params.lora_rank, alpha=params.lora_alpha,
                          dropout=params.lora_dropout).to(params.compute_dtype)
            ...
            print(f'created transformer block {layer_id}')
            logging.debug(f'created transformer block {layer_id}')

        self.norm = RMSNorm(params.dim, eps=params.norm_eps)
        self.norm.requires_grad = False
        self.output = BlackboxDisk(
            nn.Linear(params.dim, params.vocab_size, bias=False), params)

        # some useful precompute for the RoPE relative positional embeddings
        freqs_cos, freqs_sin = precompute_freqs_cis(
            self.params.dim // self.params.n_heads, self.params.max_seq_len, theta=params.rope_theta)
    ...
    def manual_loop(self, tokens, targets):
        ...
        for i, (layer, lora) in enumerate(zip(self.layers, self.lora_layers)):
            ...
            current = layer(current, freqs_cos, freqs_sin,
                            lora['q_lora'], lora['v_lora'])
            logging.log(level=logging.DEBUG,
                        msg=f'forward: transformer block {i} done')
        ...
        logging.log(level=logging.DEBUG, msg='output layer')
        logits = self.output(norm_out)
        del norm_out

        logging.log(level=logging.DEBUG, msg='output layer done')
        ...
        logging.log(level=logging.DEBUG, msg='forward: computed loss')

        loss.backward()

        norm_out_grad = self.backprop_w_lora(
            self.output, logits.grad.to(self.params.compute_dtype))
        del logits
        logging.log(level=logging.DEBUG, msg='combined: output layer done')
        ...
        for i, (layer, rng_state, lora) in enumerate(zip(reversed(self.layers), reversed(rng_before), reversed(self.lora_layers))):
            ...
            # now, do attention
            cleanup_cache(device)
            last_grad = self.backprop_w_lora(
                layer.attention, last_grad, freqs_cos, freqs_sin, lora['q_lora'], lora['v_lora'])
            logging.log(level=logging.DEBUG,
                        msg=f'combined: transformer block {i} done')
