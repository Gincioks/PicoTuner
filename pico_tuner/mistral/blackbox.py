...
class BlackboxDisk(torch.nn.Module):
    def __init__(self, module, args: MistralModelArgs):
        super().__init__()
        self.module_id = next_id(is_finetune=args.init_frozen)
        self.input_id = next_id(is_finetune=args.init_frozen)
        ...
        if args.init_frozen:
            torch.save(module.to('cpu').to(
                self.frozen_dtype), self.frozen_path())
